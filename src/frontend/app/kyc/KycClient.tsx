  ...
  useEffect(() => {
    kycApi
      ...
      .then((res) => {
        ...
        if (embedded && (status === "REJECTED" || status === "INFO_REQUIRED")) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#667085]">{t("formSubtitle")}</p>
          </div>
        </div>
        {embedded && (
          <button
      ...
      <button
        ...
        className="mt-4 inline-flex w-full items-center justify-center rounded-xl border border-[#d8aa55] px-5 py-3 text-sm font-semibold text-[#9c6507] transition hover:bg-[#fff8e9] disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto"
      >
      ...
      <button
        ...
        className="mt-6 w-full rounded-xl bg-gradient-to-r from-[#d89a27] to-[#c98509] px-7 py-3.5 text-sm font-semibold text-white shadow-[0_8px_22px_rgba(199,132,12,0.18)] disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto sm:min-w-56"
      >
