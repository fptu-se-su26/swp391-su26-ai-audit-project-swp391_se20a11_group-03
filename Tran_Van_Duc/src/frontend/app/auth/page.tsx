...
export default function AuthPage() {
  ...
  function setAuthRole(role: DemoRole) {
    document.cookie = `${AUTH_COOKIE}=${role}; path=/; max-age=604800; SameSite=Lax`;
  }
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const account = DEMO_ACCOUNTS.find(
      (demoAccount) =>
        demoAccount.email.toLowerCase() === emailOrPhone.trim().toLowerCase() &&
        demoAccount.password === password,
    );

    if (!account) {
      setLoginError("Email hoặc mật khẩu không đúng với tài khoản demo.");
      return;
    }
        ...
        <section className="grid flex-1 grid-cols-1 items-center gap-8 py-6 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-start lg:gap-8 lg:py-6 xl:grid-cols-[minmax(0,1fr)_410px]">
          ...
          <div className="flex justify-center lg:justify-end">
            ...
            >
              ...
              <button
                type="submit"
                className="mt-3 h-10 w-full rounded-lg bg-[#f0c982] text-sm font-bold tracking-wide text-black transition-colors hover:bg-[#f4d79b]"
              >
