...
export default function AdminRevenuePage() {
  ...
  const stats = [
    { label: "Doanh thu", value: `${data.summary.totalRevenue.toLocaleString("vi-VN")} ₫` },
    { label: "Giao dịch", value: totalTransactions.toLocaleString("vi-VN") },
    { label: "Người dùng", value: data.summary.totalUsers.toLocaleString("vi-VN") },
    { label: "Số dư nền tảng", value: `${data.summary.adminBalance.toLocaleString("vi-VN")} ₫` },
  ];
  return <AdminShell><div className="mx-auto max-w-7xl px-6 py-10">
    <p className="text-xs font-semibold tracking-[0.3em] text-[var(--luxora-gold)]">ADMIN CONTROL CENTER</p><h1 className="font-display-lg mt-2 text-3xl">Doanh thu &amp; Vận hành</h1>
    ...
    <div className="glass-panel mt-8 rounded-2xl p-6"><p className="mb-6 text-xs font-semibold uppercase tracking-wider text-white/40">Doanh thu theo ngày</p><div className="flex h-40 items-end gap-2">{curve.map((item) => <div key={item.date} className="flex-1 rounded-t-md bg-[var(--luxora-gold)]/60" style={{ height: `${Math.max(4, item.amount / maxValue * 100)}%` }} title={`${item.date}: ${item.amount.toLocaleString("vi-VN")} ₫`} />)}{!loading && curve.length === 0 && <p className="m-auto text-sm text-white/45">Chưa có dữ liệu doanh thu.</p>}</div></div>
    <h2 className="font-headline-md mt-10 mb-4 text-lg">Giao dịch gần đây</h2><div className="overflow-x-auto rounded-2xl border border-white/10"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b border-white/10 text-xs uppercase tracking-wider text-white/40"><th className="px-5 py-3">Lot #</th><th className="px-5 py-3">Sản phẩm / Người bán → Người mua</th><th className="px-5 py-3">Giá bán</th><th className="px-5 py-3">Ngày</th><th className="px-5 py-3">Trạng thái</th></tr></thead><tbody>{data.sales.slice(0, 8).map((row) => <tr key={row.auctionId} className="border-b border-white/5"><td className="px-5 py-4 text-[var(--luxora-gold)]">LOT-{row.productId}</td><td className="px-5 py-4"><p className="font-medium">{row.productName}</p><p className="text-xs text-white/40">{row.sellerName} → {row.buyerName}</p></td><td className="px-5 py-4 font-semibold text-[var(--luxora-gold-light)]">{row.finalPrice.toLocaleString("vi-VN")} ₫</td><td className="px-5 py-4 text-white/60">{row.paidAt ? new Intl.DateTimeFormat("vi-VN").format(new Date(row.paidAt)) : "—"}</td><td className="px-5 py-4">{row.paymentStatus ?? row.status}</td></tr>)}</tbody></table></div>
  </div></AdminShell>;
}
