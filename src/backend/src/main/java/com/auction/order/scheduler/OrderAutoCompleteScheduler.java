...
public class OrderAutoCompleteScheduler {
    ...
    @Value("${bidzone.order.auto-complete-days:3}") private long autoCompleteDays;
    @Scheduled(cron = "${bidzone.order.auto-complete-cron:0 0 * * * *}")
    public void completeDeliveredOrders() { orders.findByStatusAndDeliveredAtBefore(OrderStatus.DELIVERED, LocalDateTime.now().minusDays(autoCompleteDays)).forEach(o -> service.autoComplete(o.getOrderId())); }
}
