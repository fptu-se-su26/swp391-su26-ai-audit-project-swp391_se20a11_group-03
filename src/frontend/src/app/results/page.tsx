  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      ...
      searchProducts({
        ...
        // (e.g. recently-closed auctions we just seeded). Pass APPROVED explicitly.
        status: "APPROVED",
        auctionStatus: "ENDED",
        size: 12,
      })
          ...
          {featuredProduct && (
            <div className="hidden rounded-lg border border-white/20 bg-white/10 p-sm shadow-xl backdrop-blur md:block">
              ...
              <div className="mt-sm flex items-center justify-between text-white">
                <div>
                  <p className="font-label-sm text-label-sm text-white/60">{t("featuredLot")} #{featuredProduct.productId}</p>
                  <h2 className="font-headline-sm text-headline-sm">{featuredProduct.productName}</h2>
                </div>
                <div className="text-right">
                  <p className="font-label-sm text-label-sm text-white/60">{t("finalPrice")}</p>
                  <p className="font-headline-sm text-headline-sm">{numberFormatter.format(featuredProduct.currentBid || featuredProduct.startingPrice)} VND</p>
                </div>
              </div>
            </div>
          )}
      ...
      <section id="catalogue" className="mx-auto max-w-screen-2xl px-margin-mobile py-xl md:px-margin-desktop">
        <div className="mb-lg flex flex-col gap-xs md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">{t("completedAuctions")}</h2>
          </div>
          <div className="flex gap-sm">
            <Link href="/upcoming" className="font-label-md text-label-md text-secondary hover:underline">
              {t("viewUpcoming")}
            </Link>
        ...
        {errorMessage && <div className="mb-md rounded-lg border border-error/30 bg-error-container px-4 py-3 font-body-md text-body-md text-error">{errorMessage}</div>}

        {!isLoading && !errorMessage && products.length === 0 && (
          <div className="rounded-lg border border-dashed border-outline-variant bg-surface p-xl text-center">
            <h3 className="font-headline-sm text-headline-sm text-primary">{t("noCompletedAuctions")}</h3>
            ...
            <Link key={product.productId} href={`/auctions/${product.productId}`} className="group relative overflow-hidden rounded-lg border border-outline-variant bg-surface shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
              ...
              <div className="p-md">
                <div className="mb-xs flex items-center justify-between gap-sm">
                  ...
                  <span className="rounded-full bg-error-container px-2 py-1 font-label-sm text-[10px] uppercase tracking-wide text-on-error-container">{product.auctionStatus ?? "ENDED"}</span>
                </div>
                <h3 className="min-h-12 font-headline-sm text-headline-sm text-primary">{product.productName}</h3>
                <div className="mt-sm flex items-center justify-between">
                  <div>
                    <p className="font-label-sm text-label-sm text-on-surface-variant">{t("finalPrice")}</p>
                    <p className="font-label-lg text-label-lg text-on-surface">{numberFormatter.format(product.currentBid || product.startingPrice)} VND</p>
                  </div>
