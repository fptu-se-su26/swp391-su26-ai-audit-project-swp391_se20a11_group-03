...
export default function ApprovalsClient() {
  const t = useTranslations("staffApprovalsPage");
  ...
  const openDetail = useCallback(async (item: ReviewProduct) => {
    ...
    try {
      ...
      // Product details can still render if the category no longer exists.
    }
  }, []);
  ...
  function openApproval(item: ReviewProduct) {
    setNotice(null);
    setDetailItem(null);
  ...
  async function approve(event: FormEvent<HTMLFormElement>) {
      ...
      if (
        ...
        setApprovalError(t("leadTimeError"));
        return;
      }
      ...
      if (
        ...
        setApprovalError(t("durationError"));
        return;
      }
      ...
      try {
        ...
        setNotice(t("approveNotice", { name: approvalItem.productName }));
        closeApproval();
        reload();
      } catch (err) {
        setApprovalError(
          err instanceof ApiError ? err.message : t("approveError"),
        );
      } finally {
        setBusyId(null);
      }
  ...
  async function reject(item: ReviewProduct) {
      const reason = window.prompt(t("rejectPrompt", { name: item.productName }), "");
      if (reason === null) return;
      setNotice(null);
      setBusyId(item.productId);
      try {
        await adminApi.rejectProduct(item.productId, reason);
        setNotice(t("rejectNotice", { name: item.productName }));
        setDetailItem((current) =>
          current?.productId === item.productId ? null : current,
        );
        reload();
      } catch (err) {
        setNotice(err instanceof ApiError ? err.message : t("rejectError"));
      } finally {
        setBusyId(null);
      }
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <Stat label={t("pending")} value={items.length} color="text-yellow-300" />
        <Stat label={t("withImages")} value={items.filter((i) => i.images?.length).length} color="text-blue-300" />
        <Stat label={t("withoutImages")} value={items.filter((i) => !i.images?.length).length} color="text-red-300" />
      </div>
      ...
      <h2 className="font-headline-md mb-4 mt-8 text-lg">{t("queueTitle")}</h2>
      <div className="flex flex-col gap-3">
            ...
            <div key={item.productId} className="glass-card flex flex-wrap items-center gap-4 rounded-2xl p-4">
              ...
              <div className="min-w-[200px] flex-1">
                <p className="text-[10px] text-white/40">
                  {item.categoryName} · {item.submittedAt ? new Intl.DateTimeFormat(locale).format(new Date(item.submittedAt)) : "—"}
                </p>
                <p className="text-sm font-semibold">{item.productName}</p>
                <p className="text-xs text-white/40">Seller #{item.sellerId}</p>
              </div>
              <p className="font-semibold text-[var(--luxora-gold-light)]">
                {numberFormat.format(item.startingPrice)} ₫
              </p>
              <div>
                <button
                  type="button"
        ...
        {!loading && items.length === 0 && (
          <p className="py-12 text-center text-sm text-white/45">{error ?? t("empty")}</p>
        )}
      </div>
        ...
        >
          <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-white/10 bg-[var(--luxora-bg-elevated)] shadow-2xl">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-white/35">
                    {t("productCode", { id: detailItem.productId })}
                  </span>
                </div>
            ...
            <div className="grid gap-8 p-6 lg:grid-cols-[1.05fr_0.95fr]">
              <section aria-label={t("productImages")}>
                <div className="aspect-[4/3] overflow-hidden rounded-2xl border border-white/10 bg-black/30">
                  ...
                  <img
                    ...
                    alt={t("imageAlt", { name: detailItem.productName, number: detailImageIndex + 1 })}
                ...
                <p className="mt-2 text-xs text-white/35">
                  {detailItem.images.length > 0
                    ? t("imageCount", { count: detailItem.images.length })
                    : t("noImages")}
                </p>
              </section>
              ...
              <section>
                <div className="rounded-2xl border border-[var(--luxora-gold)]/20 bg-[var(--luxora-gold)]/5 p-5">
                  <p className="text-xs text-white/45">{t("startingPrice")}</p>
                  <p className="mt-1 text-3xl font-bold text-[var(--luxora-gold-light)]">
                    {numberFormat.format(detailItem.startingPrice)} ₫
                  </p>
                  {detailItem.stepPrice !== null && (
                    <p className="mt-2 text-xs text-white/45">
                      {t("bidStep", { amount: numberFormat.format(detailItem.stepPrice) })}
                    </p>
                  )}
                </div>

                <dl className="mt-5 grid grid-cols-2 gap-x-5 gap-y-4 rounded-2xl border border-white/10 p-5 text-sm">
                  <DetailField label={t("category")} value={detailItem.categoryName || "—"} />
                  <DetailField label={t("seller")} value={`Seller #${detailItem.sellerId}`} />
                  <DetailField
                    label={t("submittedAt")}
                    value={
                ...
                <div className="mt-5">
                  ...
                  <p className="mt-2 whitespace-pre-wrap rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-sm leading-6 text-white/65">
                    {detailItem.description?.trim() || t("noDescription")}
                  </p>
                </div>

                {detailItem.attributes?.length > 0 && (
                  <div className="mt-5">
                    <h3 className="text-sm font-semibold">{t("attributes")}</h3>
                        ...
                        >
                          <dt className="text-white/45">
                            {attributeNames[attribute.attributeId] ??
                              t("attributeFallback", { id: attribute.attributeId })}
                          </dt>
            ...
            <div className="sticky bottom-0 flex flex-col-reverse gap-3 border-t border-white/10 bg-[var(--luxora-bg-elevated)] px-6 py-4 sm:flex-row sm:justify-end">
              ...
              >
                {busyId === detailItem.productId ? t("processing") : t("reject")}
              </button>
              <button
                type="button"
        ...
        >
          ...
          >
              ...
              {approvalForm.auctionMode === "TIMED" && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm outline-none focus:border-[var(--luxora-gold)]"
                  />
                  <p className="mt-1.5 text-[11px] text-white/35">{t("durationHelp")}</p>
                </div>
              )}
            </div>
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {busyId === approvalItem.productId ? t("approving") : t("confirmApprove")}
              </button>
            </div>
          </form>
        </div>
