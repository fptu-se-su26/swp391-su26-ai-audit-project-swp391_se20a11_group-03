...
export default function PremiumPurchaseClient() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-7 lg:py-14">
      <div className="overflow-hidden rounded-[28px] border border-[#e4d7bd] bg-[radial-gradient(circle_at_top_right,rgba(215,173,78,.18),transparent_36%),linear-gradient(145deg,#fffdf9,#f7f2e9)] shadow-[0_24px_70px_rgba(80,59,25,.14)]">
        <div className="grid gap-8 p-6 lg:grid-cols-[1.1fr_.9fr] lg:p-10">
          <div>
            ...
            <h1 className="mt-5 max-w-xl text-4xl font-bold leading-tight text-[#211b13] sm:text-5xl">
              {status?.accountType === "USER" ? "Đấu giá thông minh hơn với Premium" : "Nâng tầm tài khoản người bán"}
            </h1>
            <p className="mt-4 max-w-xl text-base leading-7 text-[#71675b]">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {(status?.accountType === "USER" ? USER_BENEFITS : BENEFITS).map(([icon, title, description]) => (
                <div key={title} className="rounded-2xl border border-[#e7dfd2] bg-white/85 p-4 shadow-[0_8px_24px_rgba(69,50,20,.06)] transition hover:-translate-y-0.5 hover:border-[#d7ad4e]/55">
                  ...
                  <h2 className="mt-3 font-bold text-[#282116]">{title}</h2>
          ...
          <aside className="self-start rounded-3xl border border-[#dfc98f] bg-white/90 p-6 shadow-[0_18px_50px_rgba(96,69,19,.12)] lg:sticky lg:top-8">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setSelectedPlan("MONTHLY")} className={`rounded-xl border p-3 text-left transition ${selectedPlan === "MONTHLY" ? "border-[#c89418] bg-[#fbefcc]" : "border-[#e3d9ca] bg-white"}`}>
                <span className="block text-xs text-[#80766a]">Hàng tháng</span><strong className="mt-1 block text-[#282116]">{status ? money(status.monthlyPrice) : "--"}</strong>
              </button>
              <button type="button" onClick={() => setSelectedPlan("YEARLY")} className={`relative rounded-xl border p-3 text-left transition ${selectedPlan === "YEARLY" ? "border-[#c89418] bg-[#fbefcc]" : "border-[#e3d9ca] bg-white"}`}>
                ...
                <span className="block text-xs text-[#80766a]">Hàng năm</span><strong className="mt-1 block text-[#282116]">{status ? money(status.yearlyPrice) : "--"}</strong>
              </button>
            </div>
            <div className="mt-5 flex items-end gap-2"><strong className="text-4xl text-[#9b6a08]">{money(selectedPrice)}</strong></div>
            <p className="mt-2 text-sm text-[#7a7064]">{selectedPlan === "YEARLY" ? `12 tháng · tiết kiệm ${money(status?.yearlySaving ?? 0)}` : "Gia hạn mỗi tháng"}</p>
            <div className="my-6 h-px bg-[#e9dfcf]" />
            {loading ? <p className="text-[#7a7064]">Đang kiểm tra tài khoản...</p> : status && (
              <>
                <div className="rounded-xl bg-[#f6f1e9] px-4 py-3"><div className="flex justify-between text-sm"><span className="text-[#80766a]">Số dư khả dụng</span><span className="font-bold text-[#282116]">{money(status.remainingBalance)}</span></div></div>
                {status.premium && (
            ...
            <p className="mt-5 text-center text-xs leading-5 text-[#92887c]">Giao dịch không thể hoàn tác sau khi Premium được kích hoạt.</p>
          </aside>
        </div>
      </div>
