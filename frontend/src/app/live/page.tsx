    ...
    <main className="min-h-screen overflow-x-clip luxe-page text-[#f5ead9]">
      ...
      <section className="relative isolate overflow-hidden bg-[#070706] text-white">
        <div className="absolute inset-0 -z-10">
          <Image
            ...
            unoptimized
          />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_22%,rgba(212,170,97,.28),transparent_32%),radial-gradient(circle_at_12%_85%,rgba(201,154,75,.2),transparent_34%),linear-gradient(135deg,rgba(7,7,6,.92),rgba(16,13,8,.9)_56%,rgba(23,19,13,.92))]" />
          ...
          {featuredProduct && (
            <div className="hidden rounded-[30px] border border-white/15 bg-white/[.05] p-3 shadow-[0_30px_80px_rgba(0,0,0,.5)] backdrop-blur-xl md:block">
              ...
              <div className="mt-5 flex items-center justify-between gap-4 px-2 pb-2 text-white">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#d4aa61]">Lot #{featuredProduct.productId}</p>
                ...
                <div className="shrink-0 text-right">
                  ...
                  <p className="mt-1 text-xl font-black text-[#efcf88]">{numberFormatter.format(heroBid)} VND</p>
      ...
      <section id="catalogue" className="relative mx-auto max-w-[1440px] px-4 py-12 sm:px-6 lg:px-10 lg:py-16">
        <div className="mb-7 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[.24em] text-[#d4aa61]">{t("biddingInProgress")}</p>
            <h2 className={`${displayFont} mt-2 text-4xl font-medium leading-tight text-white`}>{t("allLiveSessions")}</h2>
          </div>
        ...
        <div className="mb-8 grid gap-4 rounded-[28px] border border-white/10 bg-white/[0.03] p-4 backdrop-blur lg:grid-cols-[1.5fr_1fr_1fr_1fr_auto]">
          <label className="block">
            <span className="mb-2 block text-[11px] font-black uppercase tracking-[.12em] text-[#9d948a]">{t("search")}</span>
            <input
              value={keyword}
          ...
          <label className="block">
            <span className="mb-2 block text-[11px] font-black uppercase tracking-[.12em] text-[#9d948a]">{t("minPrice")}</span>
            <input
              value={minPrice}
          ...
          <label className="block">
            <span className="mb-2 block text-[11px] font-black uppercase tracking-[.12em] text-[#9d948a]">{t("maxPrice")}</span>
            <input
              value={maxPrice}
          ...
          <button
            ...
            className="self-end rounded-2xl border border-white/10 px-5 py-3 text-sm font-black text-[#cfc6ba] transition hover:-translate-y-0.5 hover:border-[#d4aa61]/60 hover:text-[#d4aa61]"
          >
            {t("clear")}
          </button>
        </div>
        ...
        {errorMessage && (
          <div className="mb-6 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm font-semibold text-red-300">
            {errorMessage}
          </div>
        )}

        {!isLoading && !errorMessage && sortedProducts.length === 0 && (
          <div className="rounded-[28px] border border-dashed border-[#d4aa61]/40 bg-white/[0.03] px-6 py-16 text-center">
            <span className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-[#0e0d0b] text-[#efcf88]">
              <span className="material-symbols-outlined text-[30px]">gavel</span>
            </span>
            <h3 className={`${displayFont} mt-5 text-2xl font-medium text-white`}>{t("noLiveAuctions")}</h3>
            <p className="mt-2 text-sm text-[#b7aea3]">
              {t("checkUpcoming")} <Link href="/upcoming" className="text-[#d4aa61] hover:underline">{t("upcomingCatalogue")}</Link> {t("sessionsOpeningSoon")}
            </p>
          </div>
        )}
