...
function FeaturedLotCard({ lot }: { lot: LuxuryLot }) {
  return (
    <article className="animate-fade-up group relative overflow-hidden rounded-[28px] border border-white/15 bg-white/[0.05] p-3 shadow-[0_30px_80px_rgba(0,0,0,.5)] backdrop-blur-xl transition duration-500 hover:-translate-y-1.5 hover:border-[#d4aa61]/60">
      <div className="relative aspect-[16/11] overflow-hidden rounded-[20px] bg-[#11100d]">
        <Image
          src={lot.image}
          alt={lot.title}
          fill
          ...
          unoptimized
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/28 to-black/20" />
        <div className="absolute left-4 top-4 flex flex-wrap gap-2">
          <StatusBadge status="ACTIVE" label="Live auction" className="bg-red-50 text-red-700" />
          <span className="inline-flex items-center gap-1 rounded-full border border-white/20 bg-black/70 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.12em] text-white backdrop-blur"><span className="material-symbols-outlined text-[14px] text-[#d4aa61]">verified</span> Authenticated</span>
        </div>
        <div className="absolute inset-x-5 bottom-5 flex items-end justify-between gap-3 text-white">
          <div>
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[.2em] text-[#d4aa61]">Lot {lot.lotNumber}</p>
            <h2 className={`${displayFont} max-w-[420px] text-xl font-medium leading-tight sm:text-2xl`}>{lot.title}</h2>
          </div>
      ...
      <div className="grid gap-4 px-2 pb-2 pt-5 sm:grid-cols-[1fr_auto] sm:items-end">
        ...
        <div className="flex gap-2">
          <Link href={`/auctions/${lot.id}`} className="rounded-full bg-gradient-to-r from-[#f0ce88] to-[#c99a4b] px-5 py-3 text-center text-xs font-bold uppercase tracking-[.1em] text-[#100d08] transition hover:-translate-y-0.5 hover:brightness-110">Place bid</Link>
