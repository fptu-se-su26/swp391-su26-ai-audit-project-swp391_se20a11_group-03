...
function remaining(endTime?: string | null) {
  ...
  return days > 0 ? `${days}d ${hours}h` : `${String(hours).padStart(2, "0")}h ${String(mins).padStart(2, "0")}m`;
}

function toLuxuryLot(product: ProductSummary, index: number, status: LuxuryLot["status"]): LuxuryLot {
  const bid = product.currentBid || product.startingPrice || 0;
  return {
    id: product.productId,
    lotNumber: String(product.productId).padStart(3, "0"),
    title: product.productName,
    category: product.categoryName || "Collectibles",
    image: getProductImage(product.imageUrl),
    currentBid: bid,
    estimateLow: Math.round(bid * 1.12),
    estimateHigh: Math.round(bid * 1.35),
    timeLeft: status === "ended" ? "Sold" : remaining(product.auctionEndTime),
    bids: 12 + index * 7,
    status,
    verified: true,
  };
