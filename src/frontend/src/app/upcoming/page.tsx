      ...
      <section className="relative isolate overflow-hidden bg-[#070706] text-white">
        <div className="absolute inset-0 -z-10">
          <Image
            ...
            unoptimized
          />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_18%,rgba(212,170,97,.26),transparent_32%),radial-gradient(circle_at_8%_88%,rgba(201,154,75,.2),transparent_34%),linear-gradient(135deg,rgba(7,7,6,.94),rgba(16,13,8,.92)_58%,rgba(23,19,13,.94))]" />
      ...
      <section id="catalogue" className="relative mx-auto max-w-[1440px] px-4 py-12 sm:px-6 lg:px-10 lg:py-16">
        <div className="mb-7 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className={`${displayFont} mt-2 text-4xl font-medium leading-tight text-white`}>{t("upcomingCatalogues")}</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/results" className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-bold text-[#cfc6ba] transition hover:border-[#d4aa61]/60 hover:text-[#d4aa61]">
              {t("viewResults")}
            </Link>
        ...
        {!isLoading && !errorMessage && sortedProducts.length === 0 && (
          <div className="rounded-[28px] border border-dashed border-[#d4aa61]/40 bg-white/[0.03] p-12 text-center">
            <span className="material-symbols-outlined rounded-full bg-[#0e0d0b] p-4 text-4xl text-[#d4aa61]">event_busy</span>
            <h3 className={`${displayFont} mt-5 text-2xl font-medium text-white`}>{t("noUpcomingItems")}</h3>
